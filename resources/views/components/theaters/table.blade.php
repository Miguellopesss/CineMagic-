        ...
        @foreach($theaters as $theater)
            <tr>
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    @if($showView)
                        <a href="{{ route('theaters.show', $theater->id) }}" class="text-indigo-600 hover:text-indigo-700 dark:text-indigo-400">Show</a>
                    @endif
                    @if($showEdit)
                        <a href="{{ route('theaters.edit', $theater->id) }}" class="ml-4 text-yellow-600 hover:text-yellow-700 dark:text-yellow-400">Edit</a>
                    @endif
                </td>
            </tr>
        @endforeach

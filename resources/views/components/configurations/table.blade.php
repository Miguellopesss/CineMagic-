    ...
    <tbody class="bg-white dark:bg-neutral-600/10 divide-y divide-neutral-200 dark:divide-neutral-700">
            <tr>
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-neutral-900 dark:text-neutral-200">
                    {{ $configurations->ticket_price }}
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-neutral-500 dark:text-neutral-400">
                    {{ $configurations->registered_customer_ticket_discount }}
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    @if($showEdit)
                        <a href="{{ route('configurations.edit') }}" class="ml-4 text-yellow-600 hover:text-yellow-700 dark:text-yellow-400">Editar</a>
                    @endif
                </td>
            </tr>
        @endforeach
